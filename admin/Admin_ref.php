...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($id) {
        ...
        $sql = "UPDATE refrigerators SET name=?, brand=?, image=?, price=?, specifications=?, extra_info=? WHERE id=?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("sssissi", $name, $brand, $image, $price, $specifications, $extra_info, $id);
    } else {
        // Add new record
        $sql = "INSERT INTO refrigerators (name, brand, image, price, specifications, extra_info) VALUES (?, ?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($sql);
...
if (isset($_GET['delete'])) {
    ...
    $sql = "DELETE FROM refrigerators WHERE id=?";
    $stmt = $conn->prepare($sql);
...
// Fetch refrigerators data
$sql = "SELECT * FROM refrigerators";
...
<head>
    ...
    <title>Admin - Refrigerators</title>
...
<div class="content">
    ...
    <div class="form-container">
        <form method="post" enctype="multipart/form-data">
            ...
            <button type="submit"><?php echo $id ? 'Update Refrigerator' : 'Add Refrigerator'; ?></button>
        </form>
    </div>
    
    <!-- Table of Existing Refrigerators -->
    <table>
        <thead>
            <tr>
                <th>ID</th>
        ...
        <tbody>
            ...
            if ($result->num_rows > 0) {
                ...
                echo "<tr><td colspan='8'>No refrigerators found.</td></tr>";
            }
            ?>
        </tbody>
    </table>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        fetch(`fetch_fridge.php?id=${editId}`)
            ...
            .then(data => {
                ...
                document.querySelector('button[type="submit"]').textContent = 'Update Refrigerator';
            });
    });
    <?php endif; ?>
</script>
